...
// This file should contain the IMPLEMENTATIONS of all your graphics functions
// that were previously in kmain.c.
int abs(int n) {
    return (n > 0) ? n : -n;
}
...
void putchar(int x, int y, char c, uint32_t color) {
    // Move your full putchar implementation here.
...
void puts(int x, int y, char *str, uint32_t color) {
    // Move your full puts implementation here.
    int current_x = x;
    for (int i = 0; str[i] != '\0'; i++) {
        putchar(current_x, y, str[i], color);
